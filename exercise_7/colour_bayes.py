'''
Author: your name
Date: 2021-10-27 15:51:27
LastEditTime: 2021-10-27 19:30:59
LastEditors: Please set LastEditors
Description: In User Settings Edit
FilePath: \exercise_7\colour_bayes.py
'''
import numpy as np
import pandas as pd
...
def main(infile):
    ...
    model_rgb.fit(X_train.values, y_train.values)    
    # TODO: print model_rgb's accuracy score
    print(model_rgb.score(X_test.values, y_test.values))

    # TODO: build model_lab to predict y from X by converting to LAB colour first.
    model_lab = make_pipeline(FunctionTransformer(lambda x: rgb2lab(x.reshape(1,-1,3)).reshape(-1,3)),GaussianNB())
    model_lab.fit(X_train.values, y_train.values)
    # TODO: print model_lab's accuracy score
    print(model_lab.score(X_test.values, y_test.values))
