'''
Author: your name
Date: 2021-11-03 11:58:52
LastEditTime: 2021-11-05 14:35:57
LastEditors: Please set LastEditors
Description: In User Settings Edit
FilePath: \exercise_8\colour_predict.py
'''
import numpy as np
import pandas as pd
...
def main():
    ...
    # predict y from X by converting to LAB colour
    knn_convert_model = make_pipeline( FunctionTransformer(rgb2lab, validate=False),KNeighborsClassifier(n_neighbors=10))
    knn_convert_model.fit(X_train, y_train)
    # build random forest classifier
    rf_rgb_model = RandomForestClassifier(n_estimators = 400, max_depth = 10, min_samples_leaf=10)
    rf_rgb_model.fit(X_train, y_train)
    # predict y from X by converting to LAB colour
    rf_convert_model = make_pipeline(FunctionTransformer(rgb2lab, validate=False),RandomForestClassifier(n_estimators=400, max_depth = 10, min_samples_leaf=10))
